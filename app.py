...
def render_building_preview(building_name, floor_area, num_floors, window_ratio, renewable_pct):
    ...
    fig.add_shape(
        type="path",
        path=f"M {x1} {y0} L {x1 + depth_x} {y0 + depth_y} L {x1 + depth_x} {y1 + depth_y} L {x1} {y1} Z",
        line=dict(color="#3E5E54", width=2),
        fillcolor="#BFD4C2",
    )
    fig.add_shape(
        type="path",
        path=f"M {x0} {y1} L {x1} {y1} L {x1 + depth_x} {y1 + depth_y} L {x0 + depth_x} {y1 + depth_y} Z",
        line=dict(color="#3E5E54", width=2),
        fillcolor="#E7F0E8",
    )

    # Completed construction volume
    fig.add_shape(type="rect", x0=x0, x1=x1, y0=y0, y1=built_top, line_width=0, fillcolor="#86AF92")
    side_built_top = y0 + depth_y + (building_height * build_progress)
    fig.add_shape(
        type="path",
        path=f"M {x1} {y0} L {x1 + depth_x} {y0 + depth_y} L {x1 + depth_x} {side_built_top} L {x1} {built_top} Z",
        line_width=0,
        fillcolor="#739F85",
    )
    ...
    for level in range(1, levels):
        y_level = y0 + building_height * (level / levels)
        color = "#5F8B71" if level <= completed_levels else "#C0D0C2"
        fig.add_shape(type="line", x0=x0, x1=x1, y0=y_level, y1=y_level, line=dict(color=color, width=1))
        fig.add_shape(
            type="line",
            x0=x1,
            x1=x1 + depth_x,
            y0=y_level,
            y1=y_level + depth_y,
            line=dict(color="#88A998" if level <= completed_levels else "#CFDCD0", width=1),
        )
    ...
    for i in range(n_cols):
        for j in range(n_rows):
            ...
            fig.add_shape(
                ...
                y1=cy + wy_gap * 0.18,
                line_width=0,
                fillcolor=lit_window if is_built else dim_window,
            )

    # Roof solar panels
    if renewable_pct > 0:
        panel_count = int(np.clip(round(1 + renewable_pct / 18), 1, 7))
        panel_step = building_width / (panel_count + 1)
        for p in range(panel_count):
            px = x0 + panel_step * (p + 1)
            fig.add_shape(
                type="path",
                path=f"M {px - 0.025} {y1 + 0.005} L {px + 0.025} {y1 + 0.005} L {px + 0.04} {y1 + 0.025} L {px - 0.01} {y1 + 0.025} Z",
                line=dict(color="#38587A", width=1),
                fillcolor="#8EB7D3",
            )
